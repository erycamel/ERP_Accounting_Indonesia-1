	...
	*/
	...
	public function actionAjaxFillTreeCP()
	{
		...
		$req = Yii::app()->db->createCommand(
				"SELECT m3.id, m3.name AS text, m2.id IS NOT NULL AS hasChildren "
				. "FROM a_budget AS m2 LEFT JOIN a_budget AS m2 ON m3.id=m2.parent_id "
				. "WHERE m3.department_id = 1 AND m3.parent_id <=> $parentId "
				. "GROUP BY m3.id ORDER BY m3.code ASC"
		);
	...
	public function actionAjaxFillTreeRMG()
	{
		...
		$req = Yii::app()->db->createCommand(
				"SELECT m3.id, m3.name AS text, m2.id IS NOT NULL AS hasChildren "
				. "FROM a_budget AS m2 LEFT JOIN a_budget AS m2 ON m3.id=m2.parent_id "
				. "WHERE m3.department_id = 2 AND m3.parent_id <=> $parentId "
				. "GROUP BY m3.id ORDER BY m3.code ASC"
		);
		$children = $req->queryAll();
