...
echo '<div id="mailbox-list" class="mailbox-list" sortby="'.$sortby.'">';
...
<form id="message-list-form"
	...
	<input type="hidden" class="mailbox-count" name="ui[]"
		value="<?php echo $dataProvider->getItemCount(); ?>" /> <input
		type="hidden" class="mailbox-sortby" name="ui[]"
	...
	<div class="mailbox-clistview-container">
		...
		$this->widget('zii.widgets.CListView', array(
				...
				'itemView'=>'_list',
				'itemsTagName'=>'table',
				'template'=>'{summary}{sorter}'.$ie6br.'<div id="mailbox-items">{items}</div>{pager}',
		...
		<div style="clear: left">
			...
			<input type="submit" id="mailbox-action-restore"
				class="btn mailbox-button" name="button[restore]" value="restore" />
			<input type="submit" class="btn" name="button[delete]"
				value="delete forever" />
			<?php else: ?>
			<?php if(!$this->module->readOnly || ( $this->module->readOnly && !$this->module->isAdmin()) ): ?>
			<input type="submit" id="mailbox-action-delete"
				class="btn mailbox-button" name="button[delete]" value="delete" />
			<?php endif; ?>
			<input type="submit" id="mailbox-action-read"
				class="btn mailbox-button" name="button[read]" value="read" /> <input
				type="submit" id="mailbox-action-unread" class="btn mailbox-button"
